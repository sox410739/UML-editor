...
public class CompositionLine extends Line {
	...
	public CompositionLine(Point from, Point to) {
		super(from, to);
	}
	...
	public void draw(Graphics graphic) {
		...
		double middleLineLength = 2*arrowLength*Math.cos(Math.toRadians(arrowAngle));
		int xPoints[] = {
			this.to.getCenterX(),
			(int) (this.to.getCenterX() - arrowLength*Math.cos(rhoUp)),
			(int) (this.to.getCenterX() - middleLineLength*Math.cos(theta)),
			(int) (this.to.getCenterX() - arrowLength*Math.cos(rhoDown))
		};
		int yPoints[] = {
			this.to.getCenterY(),
			(int) (this.to.getCenterY() - arrowLength*Math.sin(rhoUp)),
			(int) (this.to.getCenterY() - middleLineLength*Math.sin(theta)),
